 ...
 */
router.get('/', requireAuthentication, asyncHandler(async (req, res) => {
    const user = req;
 ...
 */
router.post('/', requireAuthentication, validateBody(connectRepositorySchema), asyncHandler(async (req, res) => {
    const user = req;
 ...
 */
router.get('/:id/branches', requireAuthentication, asyncHandler(async (req, res) => {
    const user = req;
 ...
 */
router.post('/:id/builds', requireAuthentication, validateBody(triggerBuildSchema), asyncHandler(async (req, res) => {
    const user = req;
    const repositoryId = req.params.id;
    if (typeof repositoryId !== 'string') {
        throw new HttpError(400, 'Repository id is invalid', 'INVALID_REPOSITORY_ID');
