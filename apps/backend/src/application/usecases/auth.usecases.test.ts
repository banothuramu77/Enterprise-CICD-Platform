...
describe('auth use cases', () => {
  it('registers a user and issues tokens', async () => {
    ...
    const prisma = {
      user: {
        findUnique: async ({ where }: { where: { email: string } }) => {
          if (where.email === 'existing@example.com') return { id: 'u1', role: RoleEnum.USER };
          return null;
        },
        create: async ({ data }: { data: Record<string, unknown> }) => ({ id: 'u2', email: data.email, role: RoleEnum.USER }),
      },
      refreshToken: {
    ...
    const tokens = await registerUser(prisma, env as any, {
  ...
  it('logs in an existing user and refreshes tokens', async () => {
    ...
    const loginResult = await loginUser(prisma, env as any, {
      email: 'user@example.com',
      password: 'supersecret',
    });
    ...
    const refreshed = await refreshTokens(prisma, env as any, createRefreshToken(env as any, { sub: 'u3', tokenId: 'rt1' }));
    expect(refreshed.accessToken).toBeTruthy();
    expect(refreshed.refreshToken).toBeTruthy();

    await logoutUser(prisma, env as any, createRefreshToken(env as any, { sub: 'u3', tokenId: 'rt1' }));
    expect(createAccessToken(env as any, { sub: 'u3', role: RoleEnum.USER })).toBeTruthy();
  });
});
